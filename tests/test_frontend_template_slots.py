...
class FrontendTemplateSlotSourceTest(unittest.TestCase):
    ...
    def test_assessment_table_uses_three_slot_templates_without_legacy_groups(self) -> None:
        ...
        self.assertIn("normalizedRows.filter((row) => row.object_name.trim() !== objectName)", table_source)
        self.assertIn("removeTechnicalSectionObject(row.object_name)", table_source)
        self.assertIn("测评对象 {objectCount}", table_source)
        self.assertIn("!technical ? (", table_source)
