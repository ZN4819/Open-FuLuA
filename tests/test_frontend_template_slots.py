...
class FrontendTemplateSlotSourceTest(unittest.TestCase):
    ...
    def test_assessment_table_uses_split_record_and_score_template_slots(self) -> None:
        ...
        self.assertIn("templateSlotsForGroup", table_source)
        ...
        self.assertIn("calculateTechnicalRows", table_source)
        self.assertIn("RA_OPTIONS", table_source)
        self.assertIn("RK_OPTIONS", table_source)
        self.assertIn('className="score-output"', table_source)
        self.assertNotIn('updateMetric(index, "object_score"', table_source)
        ...
        self.assertNotIn("onBlur={() => formatObjectScore(index)}", table_source)
        ...
        self.assertIn("calculateTechnicalRows", table_source)
        self.assertIn("calculateManagementRows", table_source)
        self.assertNotIn("updateUnitScoreForUnit", table_source)
        ...
        self.assertIn("templateSlotsForUnit(recordTemplateSlots, row.unit, \"verification_record\")", table_source)
        self.assertIn("templateSlotsForGroup(recordTemplateSlots, \"score_basis\")", table_source)
        self.assertNotIn("templateSlotsForUnit(recordTemplateSlots, row.unit, \"score_basis\")", table_source)
        self.assertIn("slot.template_group === \"verification_record\" && slot.unit !== row.unit", table_source)
        self.assertNotIn("item.id === selectedSlotId && item.unit === row.unit", table_source)
    ...
    def test_assessment_table_previews_evidence_image_when_hovering_reference_label(self) -> None:
        ...
        self.assertIn("record-rich-editor", table_source)
        self.assertIn("contentEditable", table_source)
        self.assertIn("record-reference-token", table_source)
        self.assertIn("renderRecordRichTextParts", table_source)
        self.assertIn("textSelectionFromContentEditable", table_source)
        ...
        self.assertIn("record-rich-editor", styles_source)
        self.assertIn(".record-reference-token", styles_source)
        self.assertIn("color: #0068d6", styles_source)
        self.assertNotIn("record-reference-overlay", table_source)
        self.assertNotIn("record-reference-hotspot", table_source)
    ...
    def test_assessment_table_auto_sizes_record_textareas(self) -> None:
        ...
        self.assertIn("recordEditorRefs", table_source)
        self.assertIn("recordEditorRefs.current[index] = node", table_source)
        self.assertIn("record-rich-editor", styles_source)
        self.assertIn("min-height: 126px", styles_source)
        self.assertIn("white-space: pre-wrap", styles_source)
        self.assertNotIn("function autoSizeRecordTextarea", table_source)
        self.assertNotIn("recordTextareaRefs", table_source)
    ...
    def test_template_manager_is_fixed_split_slot_editor_with_config_backup(self) -> None:
        ...
        self.assertIn("sharedScoreBasisSlots", panel_source)
        self.assertIn("dedupeSharedScoreBasisSlots", panel_source)
        self.assertIn("sectionVerificationSlots", panel_source)
        self.assertIn("template-slot-global-card", panel_source)
        ...
        self.assertIn("每个单元 1 组 / 3 模板", panel_source)
        self.assertIn("A-1 至 A-4 通用", panel_source)
        self.assertNotIn("每个单元 2 组 / 6 模板", panel_source)
