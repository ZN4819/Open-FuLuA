...
- 已完成 DI-1：新增 `docx_import_jobs` 导入任务表、导入任务 CRUD 函数、导入预览 schema、`docx_importer` 服务包骨架和 `storage/imports/{job_id}/` 目录约定；统一检查脚本已通过，当前自动化测试为 63 项。
...
- 模板 profile 结构。
- 结果记录模板库结构、章节覆盖、旧图号清理、三类模板槽位种子初始化、三类模板 API、三类模板配置导入导出、录入区三类模板源码回归、模板管理面板三类槽位编辑器源码回归、T3-6 专项整体验收、DOCX 导入任务表和导入任务 CRUD。
- 样本文档结构回归。
- 新项目 8 个章节初始化。
- 已有项目列表、重新打开和删除。
- 测评行、评分结果和交叉引用保存读取。
