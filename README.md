...
- 已完成 DI-2：新增 DOCX 包读取和结构扫描器，可识别本工具导出的 editable/final DOCX、A-1 至 A-8 章节、核心表候选、表格列数和缺失章节/表格问题；统一检查脚本已通过，当前自动化测试为 69 项。
...
- 模板 profile 结构。
- 结果记录模板库结构、章节覆盖、旧图号清理、三类模板槽位种子初始化、三类模板 API、三类模板配置导入导出、录入区三类模板源码回归、模板管理面板三类槽位编辑器源码回归、T3-6 专项整体验收、DOCX 导入任务表、导入任务 CRUD 和 DOCX 结构扫描。
- 样本文档结构回归。
- 新项目 8 个章节初始化。
- 已有项目列表、重新打开和删除。
- 测评行、评分结果和交叉引用保存读取。
