...
- 已完成 DI-4：新增图片、题注和引用解析器，可提取 DOCX 内嵌 PNG/JPEG、复制到导入任务目录、读取图片尺寸/DPI、识别 `图A-x-y` 题注、恢复章节排序，并将结果记录中的图号引用替换为 `[[FIG:import:A-x-y]]` 临时 token；统一检查脚本已通过，当前自动化测试为 78 项。
