...
- 已完成 DI-3：新增核心表格解析器，可解析技术表和管理表的测评单元、测评对象、结果记录、D/A/K、符合情况和评分字段，支持纵向合并单元格继承、内容控件文本读取和空行过滤；统一检查脚本已通过，当前自动化测试为 74 项。
...
- 模板 profile 结构。
- 结果记录模板库结构、章节覆盖、旧图号清理、三类模板槽位种子初始化、三类模板 API、三类模板配置导入导出、录入区三类模板源码回归、模板管理面板三类槽位编辑器源码回归、T3-6 专项整体验收、DOCX 导入任务表、导入任务 CRUD、DOCX 结构扫描和 DOCX 核心表格解析。
- 样本文档结构回归。
- 新项目 8 个章节初始化。
- 已有项目列表、重新打开和删除。
- 测评行、评分结果和交叉引用保存读取。
