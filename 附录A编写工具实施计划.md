...
### 后续修复：DOCX 导入技术表压缩正文列映射（2026-06-23）

- 兼容技术表表头 8 列、正文 4/5 列的压缩结构。
- 压缩正文行中缺少 D/A/K 源单元格时保持为空，对象评分和单元得分读入对应评分字段。
- 已用导入测试文件确认 `IMPORT_INVALID_DAK_VALUE` 不再出现，并补充 DOCX 核心表格解析回归。
- 当前自动化测试共 95 项。
