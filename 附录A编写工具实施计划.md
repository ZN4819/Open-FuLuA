...
- 当前自动化测试共 85 项，覆盖结构化编写到导出、预览任务、批量上传、批量失败回滚、图片替换、替换失败保护、DOCX 图片分页缩放、导出模板格式对齐、三类模板槽位种子初始化、三类模板 API、三类模板配置导入导出、录入区三类模板源码回归、模板管理面板三类槽位编辑器源码回归、DOCX 导入任务基座、DOCX 结构扫描、DOCX 核心表格解析、DOCX 图片题注引用解析、DOCX 导入预览 API 和 DOCX 确认导入创建项目。
...
当前状态（2026-06-22）：已完成 DI-1 至 DI-6，导入任务表、导入任务 CRUD、导入预览 schema、服务包骨架、导入运行时目录约定、DOCX 包读取/结构扫描、核心表格解析、图片题注引用解析、导入预览 API 以及确认导入创建项目能力已落地。下一阶段进入 DI-7：前端导入向导。
