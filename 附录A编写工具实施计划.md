...
- 每个阶段完成后，必须将代码提交并推送到模板仓库 [ZN4819/Open-FuLuA](https://github.com/ZN4819/Open-FuLuA.git)。
...
- 已将前端测评表录入改为固定测评单元分组：测评单元从当前章节结果记录模板去重提取；A-1 至 A-3 技术表按章节新增或删除测评对象并自动同步到所有固定单元，新增表单位于保存按钮下方，表格上方保留已添加对象与删除入口；A-4 技术表按用户、访问控制信息、重要数据、关键业务行为四类新增对象并同步到对应测评单元，重要信息资源安全标记完整性改回单元内新增；删除对象时只移除对应测评行和行内引用，不删除已上传证据图片；A-5 至 A-8 管理表仍在对应单元下新增测评对象。
...
- 当前自动化测试共 95 项，覆盖结构化编写到导出、预览任务、批量上传、批量失败回滚、图片替换、替换失败保护、DOCX 图片分页缩放、导出模板格式对齐、三类模板槽位种子初始化、三类模板 API、三类模板配置导入导出、录入区三类模板源码回归、模板管理面板三类槽位编辑器源码回归、DOCX 导入任务基座、DOCX 结构扫描、DOCX 核心表格解析、DOCX 图片题注引用解析、DOCX 导入预览 API、DOCX 确认导入创建项目、前端 DOCX 导入向导和 editable/final DOCX 导入 roundtrip。
...
- 已补充固定测评单元录入：测评单元第一列改为只读分组；A-1 至 A-3 技术表新增对象表单位于保存按钮下方，表格上方保留已添加对象与删除入口，并作用于本章所有固定单元；A-4 技术表按用户、访问控制信息、重要数据、关键业务行为分类新增对象，重要信息资源安全标记完整性单元内新增对象；管理表每个固定单元内可新增测评对象，模板下拉仅显示本单元模板；技术表删除对象不删除已上传证据图片。
...
当前状态（2026-06-23）：已完成 DI-1 至 DI-8，导入任务表、导入任务 CRUD、导入预览 schema、服务包骨架、导入运行时目录约定、DOCX 包读取/结构扫描、核心表格解析、图片题注引用解析、导入预览 API、确认导入创建项目、前端导入向导和 editable/final DOCX roundtrip 端到端回归已落地。DOCX 导入新项目专项第一版已完成。
...
CD-1 至 CD-7 的构建、运行时目录、同源托管、侧车生命周期、迁移恢复、NSIS 和升级恢复链路已进入 CD-8 全量验收。CD-8 新增可复现的安装包业务验收脚本，并同步安装、升级、发布、故障排查和验收记录。

交付判定以实际运行证据为准：自动化覆盖新建、保存、图片上传、校验、editable/final 导出、DOCX 再导入、关闭重开、迁移和卸载重装；图片粘贴、干净 Windows 环境、签名信任链和真实跨版本升级仍需独立人工或发布环境证据。无签名证书时只准备 `0.1.0-rc.1` 未签名预发布候选，不提前创建稳定版标签。
### 后续修复：测评单元得分自动计算（2026-06-23）

- 技术表单元得分由同一测评单元下的对象评分自动计算平均值，结果保留四位小数。
- 对象评分为 `/` 的测评对象不纳入平均值；若同一测评单元下所有对象评分均为 `/`，单元得分为 `/`。
- 录入表单元得分列按测评单元合并显示；管理表保留合并后的单元得分录入能力。
- DOCX 导出同步合并测评单元得分列，并在导出前重新计算技术表单元得分。
- 已补充保存归一化、`/` 评分校验、DOCX 合并和前端源码级回归，当前自动化测试共 95 项。

### 后续修复：对象评分四位小数格式化（2026-06-23）

- 技术表对象评分在前端输入框失焦时自动格式化为四位小数。
- 后端保存章节行和 DOCX 导出会再次规范对象评分，确保接口写入、刷新和导出结果一致。
- `/` 评分保持原样，不转为数字；非法评分不强制改写，继续由校验服务提示。
- 单元得分平均值基于规范后的对象评分计算，避免对象评分显示值和单元得分末位不一致。
- 已补充结构化保存、DOCX 导出和前端源码级回归，当前自动化测试共 95 项。

### 后续修复：DOCX 导入技术表内容控件包裹单元格映射（2026-06-23）
...
后续升级采用“保留附录A旧模式、新增完整报告模式”的兼容路线。详细、可提交、可验收的阶段方案集中存放于 [密评报告工具升级实施方案索引](docs/report-tool/README.md)。

阶段顺序：

1. [R0：模板治理与运行时母版](docs/report-tool/R0-模板治理与运行时母版实施方案.md)
2. [R1：双项目模式与兼容迁移](docs/report-tool/R1-双项目模式与兼容迁移实施方案.md)
3. [R2：完整报告数据域与章节工作台](docs/report-tool/R2-完整报告数据域与章节工作台实施方案.md)
4. [R3：正文生成与一致性链路](docs/report-tool/R3-正文生成与一致性链路实施方案.md)
5. [R4：完整报告 DOCX 装配](docs/report-tool/R4-完整报告DOCX装配实施方案.md)
6. [R5：附录B证据模块](docs/report-tool/R5-附录B证据模块实施方案.md)
7. [R6：既有完整报告迁移](docs/report-tool/R6-既有完整报告迁移实施方案.md)
8. [R7：受控 Word 回收](docs/report-tool/R7-受控Word回收实施方案.md)
9. [R8：客户端更名、打包与交付](docs/report-tool/R8-客户端更名打包与交付实施方案.md)

当前状态：R0 已最终冻结并通过 G0；R1 已完成技术实施、独立代码审查修复和本地验收，数据库 schema 升至 `4`，双项目身份、不可变模板绑定、可恢复附录 A 复制升级，以及 schema 3 冷启动备份/迁移/故障恢复和安装版数据保留链路已经落地，待本阶段 Pull Request 合入 `main`。R2 至 R8 尚未开始；当前仍不具备完整报告正文数据域或完整报告导出能力。

统一交付规则：

- 每个阶段从最新 `main` 创建独立 `codex/report-rN-*` 分支。
- 一个阶段对应一个 Pull Request；阶段内每个可独立验证和回滚的工作包对应一个清晰提交。
- 每个阶段完成后运行阶段测试和 `scripts/run_checks.ps1`，更新专题索引状态、schema 版本、模板哈希和 PR 链接。
- 不得提交真实报告、源模板、源打分表、数据库、storage、日志、备份、`.codex/` 或临时生成物。
- 不使用 superpowers 插件或相关流程。
