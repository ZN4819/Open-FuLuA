...
- 当前自动化测试共 63 项，覆盖结构化编写到导出、预览任务、批量上传、批量失败回滚、图片替换、替换失败保护、DOCX 图片分页缩放、导出模板格式对齐、三类模板槽位种子初始化、三类模板 API、三类模板配置导入导出、录入区三类模板源码回归、模板管理面板三类槽位编辑器源码回归和 DOCX 导入任务基座。
...
当前状态（2026-06-22）：已完成 DI-1，导入任务表、导入任务 CRUD、导入预览 schema、服务包骨架和导入运行时目录约定已落地。下一阶段进入 DI-2：DOCX 包读取与文档结构扫描。
