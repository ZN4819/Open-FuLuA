...
- 当前自动化测试共 90 项，覆盖结构化编写到导出、预览任务、批量上传、批量失败回滚、图片替换、替换失败保护、DOCX 图片分页缩放、导出模板格式对齐、三类模板槽位种子初始化、三类模板 API、三类模板配置导入导出、录入区三类模板源码回归、模板管理面板三类槽位编辑器源码回归、DOCX 导入任务基座、DOCX 结构扫描、DOCX 核心表格解析、DOCX 图片题注引用解析、DOCX 导入预览 API、DOCX 确认导入创建项目、前端 DOCX 导入向导和 editable/final DOCX 导入 roundtrip。
