...
- 已将前端测评表录入改为固定测评单元分组：测评单元从当前章节结果记录模板去重提取；A-1 至 A-4 技术表按章节新增或删除测评对象并自动同步到所有固定单元，删除对象时只移除同名测评行和行内引用，不删除已上传证据图片；A-5 至 A-8 管理表仍在对应单元下新增测评对象。
...
- 已补充固定测评单元录入：测评单元第一列改为只读分组；技术表新增和删除对象入口位于表格上方并作用于本章所有固定单元，管理表每个固定单元内可新增测评对象，模板下拉仅显示本单元模板；技术表删除对象不删除已上传证据图片。
