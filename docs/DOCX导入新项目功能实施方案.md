...
当前进度（2026-06-22）：DI-1 至 DI-4 已完成。后端已新增 `docx_import_jobs` 导入任务表，用于记录上传文件、解析结果、导入摘要、问题清单和最终创建的项目 ID；已新增导入任务 CRUD 函数、导入预览 schema、`backend/app/services/docx_importer/` 服务包骨架和 `storage/imports/{job_id}/` 目录约定。DI-2 已新增 DOCX 包读取与文档结构扫描能力，可安全读取 OpenXML 包、解析关系文件和媒体清单、按 body 顺序识别总标题、章节标题、表题和核心表候选，并对缺失章节/表格生成问题清单。DI-3 已新增核心表格解析能力，可将技术表和管理表恢复为结构化测评行，支持纵向合并测评单元继承、内容控件选中值读取、空白占位行过滤和行级导入问题提示。DI-4 已新增图片、题注和引用解析能力，可提取内嵌 PNG/JPEG、复制到导入任务目录、读取图片尺寸/DPI、识别图题注与排序，并将结果记录中的图号引用替换为导入临时 token；统一检查脚本已通过，当前自动化测试为 78 项。当前尚未开始 DI-5 的导入预览 API。
