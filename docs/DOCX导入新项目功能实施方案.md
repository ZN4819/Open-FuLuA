...
- 导入后项目可以保存、校验和导出 editable/final DOCX。
- 导入问题清单能够说明无法解析或疑似丢失的内容。
- 原始上传 DOCX 和原始样本文档不会被覆盖或修改。

## 13. 当前进度

当前进度（2026-06-23）：DI-1 至 DI-8 已完成。后端已新增 `docx_import_jobs` 导入任务表，用于记录上传文件、解析结果、导入摘要、问题清单和最终创建的项目 ID；已新增导入任务 CRUD 函数、导入预览 schema、`backend/app/services/docx_importer/` 服务包骨架和 `storage/imports/{job_id}/` 目录约定。DI-2 已新增 DOCX 包读取与文档结构扫描能力，可安全读取 OpenXML 包、解析关系文件和媒体清单、按 body 顺序识别总标题、章节标题、表题和核心表候选，并对缺失章节/表格生成问题清单。DI-3 已新增核心表格解析能力，可将技术表和管理表恢复为结构化测评行，支持纵向合并测评单元继承、内容控件选中值读取、空白占位行过滤和行级导入问题提示。DI-4 已新增图片、题注和引用解析能力，可提取内嵌 PNG/JPEG、复制到导入任务目录、读取图片尺寸/DPI、识别图题注与排序，并将结果记录中的图号引用替换为导入临时 token。DI-5 已新增 `/api/imports/docx` 和 `/api/imports/{job_id}`，支持上传 DOCX 后保存源文件、生成 `parsed.json`、返回章节/行数/图片/引用/问题清单，并根据错误等级返回 `can_create_project`。DI-6 已新增 `POST /api/imports/{job_id}/project`，支持按预览结果事务性创建新项目、复制临时图片、写入测评行/评分/证据图片/交叉引用、替换真实图片 token，并在失败时回滚数据库和清理运行时文件。DI-7 已新增前端导入向导，首页支持选择 DOCX、上传解析、查看章节/测评行/图片/引用摘要和问题清单、编辑新项目名称、确认创建并自动打开项目。DI-8 已新增 editable/final DOCX 导入 roundtrip 端到端回归和专项验收记录，确认本工具导出的 DOCX 可重新导入为新项目，导入后可继续校验和导出；当前自动化测试为 95 项。DOCX 导入新项目专项第一版已完成。
