...
> 状态：已实施并完成阶段验收。实现证据见 [R7 实施验收记录](R7-实施验收记录.md)。
...
- 用户明确删除项目时，必须物理删除项目业务数据、导出快照、签发清单、审计明细、上传 DOCX 和公开 storage 文件；只允许保留不含正文、字段值、人员、路径或文件名的不可变哈希墓碑，用于证明历史签发/提交链曾存在。
...
- 桌面侧车存在会话令牌时，全部变更型 `/api/*` 请求均必须验证令牌；Electron 只向当前本机侧车同源 API 注入该请求头，不得发送给外部 URL。
- 上传私有区必须同时具备单文件上限、总容量上限、失败临时文件原子清理和失败源文件保留数量上限，避免无效 DOCX 持续占满本机磁盘。

## 16. 阶段验收闸门

- [x] roundtrip 必须通过 manifest 签名、服务端快照和项目归属三重同源校验。
- [x] 只有 `roundtrip_capable` 同源草稿可回收；final、普通草稿和外部 DOCX 全部拒绝。
- [x] 受控草稿恰好包含一个固定 schema 的签名 custom XML；其他档位不含 custom XML，额外部件全部拒绝。
- [x] migration 与 roundtrip 行为明确隔离。
- [x] 只回收白名单字段和已有业务行。
- [x] 新增、删除、复制、重排、拆分或合并行全部阻止。
- [x] 图片变化不回收，并有明确 warning。
- [x] Word 中出现 Ra/Rk 业务字段时阻止。
- [x] 模板包常量、R3 派生值、只读投影及附录 A 对象分/单元分/最终分值不得直接回收。
- [x] 同一权威字段的多个 Word 槽位不一致时明确报冲突，不按位置选值。
- [x] 所有未接受修订类型均能检测并阻止。
- [x] 三方 diff 规则组合测试全覆盖。
- [x] 冲突必须人工选择，解决计划绑定 diff hash 和项目 revision。
- [x] 原子事务失败时没有部分写入。
- [x] 写回后评分、结果修正和 R3 派生结果由后端重算，受影响人工正文按契约失效或待重新确认。
- [x] 每次提交都有 before/after hash 和可追溯审计。
- [x] 异常 Word 或丢失 custom XML 不会触发位置猜测；WPS/LibreOffice 重写文件明确拒绝回收。
- [x] 桌面端端到端闭环和独立代码审查通过。
